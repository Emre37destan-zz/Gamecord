...
class SnakeGame {
    constructor(options = {}) {
        ...
        if (typeof options.embed.OverTitle !== 'string')  throw new TypeError('INVALID_OVER_TITLE: Başlık üzerinde bir dize olmalıdır.')

        
        if (!options.snake) options.snake = {};
        if (!options.snake.head) options.snake.head = '🟢';
        if (typeof options.snake.head !== 'string')  throw new TypeError('INVALID_EMOJI: Yılan Kafa Emojisi bir dize olmalı.')

        if (!options.snake.body) options.snake.body = '🟩';
        if (typeof options.snake.body !== 'string')  throw new TypeError('INVALID_EMOJI: Yılan Vücut Emojisi bir dize olmalıdır.')

        if (!options.snake.tail) options.snake.tail = '🟢';
        if (typeof options.snake.tail !== 'string')  throw new TypeError('INVALID_EMOJI: Yılan Kuyruğu Emojisi bir dize olmalı.')

        if (!options.emojis) options.emojis = {};
        if (!options.emojis.board) options.emojis.board = '⬛';
        if (typeof options.emojis.board !== 'string')  throw new TypeError('INVALID_EMOJI: Pano Emojisi bir dize olmalıdır.')

        if (!options.emojis.food) options.emojis.food = '🍎';
        if (typeof options.emojis.food !== 'string')  throw new TypeError('INVALID_EMOJI: Yemek Emojisi bir dize olmalı.')
    ...
    async startGame() {
        ...
        .setDescription(`**Puan:** ${this.score}\n\n${this.getGameBoard()}`)
        ...
        let stop = new MessageButton().setLabel('DurDur').setStyle('DANGER').setCustomId(btn_stop)
